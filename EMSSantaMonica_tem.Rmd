...
dat_net.2010 <- st_join(emergencies %>% 
          filter(is.na(longitude) == FALSE &
                   is.na(latitude) == FALSE) %>%
          st_as_sf(., coords = c("longitude", "latitude"), crs = 4326),
        fishnet %>%
...
study.panel <- 
  expand.grid(interval60=unique(dat_net$interval60),
              reporting_district = unique(dat_net$reporting_district)) %>%
  left_join(., dat_net %>%
              dplyr::select(reporting_district, longitude, latitude)%>% 
              distinct() %>%
...
study.panel <- 
  ...
  group_by(interval60, reporting_district, longitude, latitude) %>% 
